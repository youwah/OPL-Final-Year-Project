...
if (isset($_POST['update_user'])){
    $sqlupdateuser="UPDATE tbl_272043_supervisor SET fname='".strtoupper($fname)."',lname='".strtoupper($lname)."',phone_number='$phone_number',ic_number='$ic_number' WHERE email='$email'";
    if(mysqli_query($conn, $sqlupdateuser)){
        
        echo "<script type='text/javascript'>alert('Success!');window.location.assign('supprofile.php');</script>'";
        
    }else{
        
        echo "<script type='text/javascript'>alert('Opps... profile update failed...');window.location.assign('supprofile.php');</script>'";
    ...
    if(!isset($file)){
            echo "<script type='text/javascript'>alert('Please upload an image');window.location.assign('supprofile.php');</script>'";
    }else{
        $image= addslashes(file_get_contents($_FILES['image']['tmp_name'])); 
        $image_size = getimagesize($_FILES['image']['tmp_name']);
        
        $query = "UPDATE `tbl_272043_supervisor` SET pic = '$image' WHERE email = '$email' ";
            if(mysqli_query($conn, $query))
            {
                echo "<script type='text/javascript'>alert('Success!');window.location.assign('supprofile.php');</script>'";
            }else{
                echo "<script type='text/javascript'>alert('Failed!!');window.location.assign('supprofile.php');</script>'";
            }
    }
}
?>
...
<div class="header"style="position:relative">
 ...
 <div class="topnav mb-3 mt-4">
	...
	<div style="height: 350px; width:80%;background-color: white;max-width:1920px; min-width:480px; margin-center:auto; margin-right:auto; margin:auto; padding:auto; overflow:hidden;float:center">
		<div style="margin-left: 10px;margin-top: 10px; background-color: white; height: 50px; width:250px;float:left"><i class="material-icons" style="float: left;margin-top: 5px;margin-left: 5px; font-size: 40px">&#xe7fd;</i><span class="discover" style="margin-left: 10px;margin-top: 13px">Supervisor Profile</span></div>
		<br><br>
		<?php
	    session_start();
        error_reporting(0);
	    ...
	    $sqlloaduser = "SELECT * FROM tbl_272043_supervisor WHERE email='$email'";
	...
	<form method="post">
		...
		<div style="width: 500px; float: left">
			<span class="label" style="float: left">Full Name:</span> 
			<input type="text" style="height:20px; width: 300px; float: right" name="firstname" value="<?php echo $lname ?>" required>
		</div>
		<div style="width: 560px; float:left">
			<span class="label" style="float: left; padding-left: 20px">Race:</span> <input type="text" style="height:20px; width: 300px; float: right " name="lastname" value="<?php echo $fname ?>" required>
		</div>	
	</div>
	<div class="bigbox">
		<div style="width: 500px; float: left">
