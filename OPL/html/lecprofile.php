...
if (isset($_POST['update_user'])){
    $sqlupdateuser="UPDATE tbl_272043_lecturer SET fname='".strtoupper($fname)."',lname='".strtoupper($lname)."',phone_number='$phone_number',ic_number='$ic_number' WHERE email='$email'";
    if(mysqli_query($conn, $sqlupdateuser)){
        
        echo "<script type='text/javascript'>alert('Success!');window.location.assign('lecprofile.php');</script>'";
        
    }else{
        
        echo "<script type='text/javascript'>alert('Opps... profile update failed...');window.location.assign('lecprofile.php');</script>'";
    ...
    if(!isset($file)){
            echo "<script type='text/javascript'>alert('Please upload an image');window.location.assign('lecprofile.php');</script>'";
    }else{
        $image= addslashes(file_get_contents($_FILES['image']['tmp_name'])); 
        $image_size = getimagesize($_FILES['image']['tmp_name']);
        
        $query = "UPDATE `tbl_lecturer` SET pic = '$image' WHERE email = '$email' ";
            if(mysqli_query($conn, $query))
            {
                echo "<script type='text/javascript'>alert('Success!');window.location.assign('lecprofile.php');</script>'";
            }else{
                echo "<script type='text/javascript'>alert('Failed!!');window.location.assign('lecprofile.php');</script>'";
            }
    }
}
?>
...
<div class="header"style="position:relative">
 ...
 <div class="topnav mb-3 mt-4">
	...
	<div style="height: 350px; width:80%;background-color: white;max-width:1920px; min-width:480px; margin-center:auto; margin-right:auto; margin:auto; padding:auto; overflow:hidden;float:center">
		<div style="margin-left: 10px;margin-top: 10px; background-color: white; height: 50px; width:250px;float:left"><i class="material-icons" style="float: left;margin-top: 5px;margin-left: 5px; font-size: 40px">&#xe7fd;</i><span class="discover" style="margin-left: 10px;margin-top: 13px">Lecturer Profile</span></div>
		<br><br>
		<?php
	    session_start();
        error_reporting(0);
